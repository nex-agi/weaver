...
"""Test 1: LoRA save_state / load_state checkpoint round-trip.

Phase 1 - New session: train 3 steps, save_state, terminate.
Phase 2 - Brand new session & model: load_state, train 3 more steps.
Verify: combined 6-step losses ≈ baseline LoRA 6-step losses.
"""
...
def main() -> None:
    ...
    # ==================================================================
    # Phase 1: Fresh session — train 3 steps, save checkpoint, terminate
    # ==================================================================
    print("=" * 60)
    print("Phase 1: LoRA train 3 steps + save_state (new session)")
    print("=" * 60)

    with ServiceClient(api_key=os.getenv("WEAVER_API_KEY")) as client:
        tc = client.create_model(base_model="Qwen/Qwen3-8B")
        ...
        # Save checkpoint
        checkpoint = tc.save_state(name="lora-checkpoint-test1")
        ...
        # ServiceClient.__exit__ terminates the model — phase 1 fully done

    print(f"\n  Phase 1 complete. Saved checkpoint path: {saved_checkpoint_path}")

    # ==================================================================
    # Phase 2: Brand new session & model — load checkpoint, train 3 steps
    # ==================================================================
    print()
    print("=" * 60)
    print("Phase 2: New session + new model + load_state + train 3 steps")
    print("=" * 60)

    with ServiceClient(api_key=os.getenv("WEAVER_API_KEY")) as client:
        tc2 = client.create_model(base_model="Qwen/Qwen3-8B")
        tokenizer = tc2.get_tokenizer()
        data = [process_example(ex, tokenizer) for ex in EXAMPLES]

        # Trigger provisioning with a forward_backward (starts the trainer pod)
    ...
    print()
    print("=" * 60)
    print("TEST 1 RESULTS: LoRA save_state / load_state")
    print("=" * 60)
    all_losses = phase1_losses + phase2_losses
    for i, loss in enumerate(all_losses):
        print(f"  Step {i}: loss/token={loss:.6f}")
    print()
    print("Compare with baseline LoRA 6-step losses.")
